...
const (
	...
	holdoutOpeningSchema  = "protocol-v2.holdout-opening.v1"
	finalCheckpointSchema = "protocol-v2.final-checkpoint.v1"
)
...
type FinalReport struct {
	SchemaVersion string                  `json:"schema_version"`
	ExperimentID  protocolv2.ExperimentID `json:"experiment_id"`
	BundleSHA256  protocolv2.SHA256Hex    `json:"bundle_sha256"`
	SourceHash    protocolv2.SHA256Hex    `json:"source_hash"`
	DataHash      protocolv2.SHA256Hex    `json:"data_hash"`
	CompletedAt   time.Time               `json:"completed_at"`
...
// Final opens a valid freeze bundle once and resumes that same opening after an
// interruption. Every holdout unit is atomically checkpointed before the next
// one starts, so a retry never creates a second scientific observation.
func Final(ctx context.Context, outputDir string, m manifest.Manifest, sourceHash, dataHash protocolv2.SHA256Hex, runner Runner, candles CandleStore, progressCallbacks ...func(Progress)) (FinalReport, error) {
	if runner == nil {
	...
	developmentPath := filepath.Join(protocolv2.ReportDir(root), "development.json")
	developmentRaw, development, err := loadDevelopmentReport(developmentPath, m, sourceHash, dataHash)
	...
	bundleHash := digest(raw)
	opening := filepath.Join(protocolv2.HoldoutDir(root), "opened.json")
	if err := openOrResumeHoldout(opening, m.ID, bundleHash, sourceHash, dataHash); err != nil {
		return FinalReport{}, err
	}
	finalPath := filepath.Join(protocolv2.HoldoutDir(root), "final.json")
	if completed, ok, err := readCompletedFinal(finalPath, m.ID, bundleHash, sourceHash, dataHash); err != nil {
		return FinalReport{}, err
	} else if ok {
	...
	final := FinalReport{
		SchemaVersion: "protocol-v2.final.v1", ExperimentID: m.ID, BundleSHA256: bundleHash,
		SourceHash: sourceHash, DataHash: dataHash,
	}
	var progress func(Progress)
	if len(progressCallbacks) > 0 {
		progress = progressCallbacks[0]
	}
	totalUnits, completedUnits := len(bundle.FinalCandidates)*2, 0
	for _, candidate := range bundle.FinalCandidates {
		...
		candidateUnits := make(map[protocolv2.CostProfileID]FinalUnitArtifact, 2)
		for _, cost := range []protocolv2.CostProfileID{m.Execution.ID, "stress"} {
			...
			artifact, reused, err := executeFinalUnit(ctx, root, m.ID, bundleHash, sourceHash, dataHash, runner, unit)
			if err != nil {
				notify(progress, Progress{Completed: completedUnits, Remaining: totalUnits - completedUnits, Unit: unit, Err: err})
				return FinalReport{}, fmt.Errorf("orchestration: final %s: %w", unit.Key(), err)
			}
			completedUnits++
			notify(progress, Progress{Completed: completedUnits, Remaining: totalUnits - completedUnits, Reused: reused, Unit: unit})
			final.Units = append(final.Units, artifact)
			candidateUnits[cost] = artifact
			releaseResearchMemory()
		}
		input, err := buildGateInput(root, m, development, candidate, candidateUnits)
		if err != nil {
			return FinalReport{}, err
		}
	...
	finalHash, err := writeJSONAtomic(finalPath, final)
	if err != nil {
		return FinalReport{}, err
	}
	if err := writeAtomic(finalPath+".sha256", []byte(finalHash+"\n")); err != nil {
		return FinalReport{}, err
	}
	return final, nil
}

func openOrResumeHoldout(path string, experimentID protocolv2.ExperimentID, bundleHash, sourceHash, dataHash protocolv2.SHA256Hex) error {
	expected := HoldoutOpening{
		SchemaVersion: holdoutOpeningSchema, ExperimentID: experimentID, BundleSHA256: bundleHash,
		SourceHash: sourceHash, DataHash: dataHash, OpenedAt: time.Now().UTC(),
	}
	if err := writeJSONExclusive(path, expected); err == nil {
		return nil
	...
	var existing HoldoutOpening
	if err := json.Unmarshal(raw, &existing); err != nil || existing.SchemaVersion != expected.SchemaVersion ||
		existing.ExperimentID != experimentID || existing.BundleSHA256 != bundleHash ||
		existing.SourceHash != sourceHash || existing.DataHash != dataHash || existing.OpenedAt.IsZero() {
		return fmt.Errorf("orchestration: holdout opening record is invalid or belongs to another frozen run")
	}
	return nil
}

func readCompletedFinal(path string, experimentID protocolv2.ExperimentID, bundleHash, sourceHash, dataHash protocolv2.SHA256Hex) (FinalReport, bool, error) {
	raw, err := os.ReadFile(path)
	...
	if strings.TrimSpace(string(checksum)) != string(digest(raw)) {
		return FinalReport{}, false, fmt.Errorf("orchestration: existing final report checksum mismatch")
	}
	var report FinalReport
	if err := json.Unmarshal(raw, &report); err != nil || report.SchemaVersion != "protocol-v2.final.v1" || report.ExperimentID != experimentID || report.CompletedAt.IsZero() {
		return FinalReport{}, false, fmt.Errorf("orchestration: existing final report is invalid")
	}
	if report.BundleSHA256 != bundleHash || report.SourceHash != sourceHash || report.DataHash != dataHash {
		return FinalReport{}, false, fmt.Errorf("orchestration: existing final report belongs to another frozen run")
	}
	return report, true, nil
}

func executeFinalUnit(ctx context.Context, root string, experimentID protocolv2.ExperimentID, bundleHash, sourceHash, dataHash protocolv2.SHA256Hex, runner Runner, unit Unit) (FinalUnitArtifact, bool, error) {
	dir := filepath.Join(protocolv2.HoldoutDir(root), "checkpoints")
	metadataPath := filepath.Join(dir, unit.Key()+".json")
	artifactPath := metadataPath + ".artifact"
	if raw, err := os.ReadFile(metadataPath); err == nil {
		var checkpoint FinalCheckpoint
		if err := json.Unmarshal(raw, &checkpoint); err != nil || checkpoint.SchemaVersion != finalCheckpointSchema ||
			checkpoint.ExperimentID != experimentID || checkpoint.BundleSHA256 != bundleHash ||
			checkpoint.SourceHash != sourceHash || checkpoint.DataHash != dataHash || checkpoint.Unit.Key() != unit.Key() {
			return FinalUnitArtifact{}, false, fmt.Errorf("stale final checkpoint metadata")
		}
		artifact, err := readCheckpointBytes(artifactPath, checkpoint.ArtifactSHA256)
		if err != nil {
			return FinalUnitArtifact{}, false, err
		}
		if err := validateUnitArtifact(unit, artifact); err != nil {
			return FinalUnitArtifact{}, false, err
		}
		return FinalUnitArtifact{Unit: unit, ArtifactSHA256: checkpoint.ArtifactSHA256, Path: artifactPath}, true, nil
	...
	hash, err := writeCompressedArtifact(artifactPath, artifact)
	if err != nil {
		return FinalUnitArtifact{}, false, err
	}
	checkpoint := FinalCheckpoint{
		SchemaVersion: finalCheckpointSchema, ExperimentID: experimentID, BundleSHA256: bundleHash,
		SourceHash: sourceHash, DataHash: dataHash, Unit: unit, ArtifactSHA256: hash, CompletedAt: time.Now().UTC(),
	}
	if _, err := writeJSONAtomic(metadataPath, checkpoint); err != nil {
		return FinalUnitArtifact{}, false, err
	}
	return FinalUnitArtifact{Unit: unit, ArtifactSHA256: hash, Path: artifactPath}, false, nil
}
...
func buildGateInput(root string, m manifest.Manifest, development DevelopmentReport, candidate FinalCandidate, holdout map[protocolv2.CostProfileID]FinalUnitArtifact) (validation.GateInput, error) {
	base := newEvidenceAccumulator()
	stressPnL := 0.0
	positiveFolds := 0
	developmentFolds := 0
	for _, checkpoint := range development.Units {
		...
		switch unit.Cost {
		case m.Execution.ID:
			base.add(artifact, m.Risk.InitialEquity)
			developmentFolds++
			if unitNetPnL(artifact, m.Risk.InitialEquity) > 0 {
				positiveFolds++
			}
		case "stress":
			stressPnL += unitNetPnL(artifact, m.Risk.InitialEquity)
		}
		releaseResearchMemory()
	}
	baseArtifact, ok := holdout[m.Execution.ID]
	if !ok {
		return validation.GateInput{}, fmt.Errorf("orchestration: missing base holdout for %s", candidate.Strategy)
	}
	baseHoldout, err := readFinalUnitArtifact(baseArtifact)
	if err != nil {
		return validation.GateInput{}, err
	}
	holdoutConsistent := unitNetPnL(baseHoldout, m.Risk.InitialEquity) > 0
	base.add(baseHoldout, m.Risk.InitialEquity)
	releaseResearchMemory()

	stressArtifact, ok := holdout["stress"]
	if !ok {
		return validation.GateInput{}, fmt.Errorf("orchestration: missing stress holdout for %s", candidate.Strategy)
	}
	stressHoldout, err := readFinalUnitArtifact(stressArtifact)
	if err != nil {
		return validation.GateInput{}, err
	}
	stressPnL += unitNetPnL(stressHoldout, m.Risk.InitialEquity)
	releaseResearchMemory()
	...
	if developmentFolds > 0 {
		positiveFraction = float64(positiveFolds) / float64(developmentFolds)
	}
	return validation.GateInput{
		AggregateTrades:       aggregate.ClosedTrades,
		EligibleSymbols:       aggregate.EligibleSymbols,
		DevelopmentFolds:      developmentFolds,
		PositiveFoldFraction:  positiveFraction,
		ProfitFactor:          aggregate.ProfitFactor,
		Expectancy:            aggregate.Expectancy,
		MedianDrawdownPercent: aggregate.MedianDrawdownPercent,
		MaxContributionPct:    aggregate.MaxContributionPct,
		StressPositive:        stressPnL > 0,
		ParameterStable:       candidate.Stability.Stable,
		NeighborsRobust:       neighborsRobust,
		HoldoutConsistent:     holdoutConsistent,
	}, nil
}

func readFinalUnitArtifact(artifact FinalUnitArtifact) (UnitResult, error) {
	raw, err := readCheckpointBytes(artifact.Path, artifact.ArtifactSHA256)
	if err != nil {
		return UnitResult{}, fmt.Errorf("orchestration: read final artifact %s: %w", artifact.Unit.Key(), err)
	}
	var result UnitResult
	if err := json.Unmarshal(raw, &result); err != nil || validateUnitResult(artifact.Unit, result) != nil {
		return UnitResult{}, fmt.Errorf("orchestration: invalid final artifact for %s", artifact.Unit.Key())
	}
	return result, nil
}
