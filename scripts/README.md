...
Для проверки event-driven гипотезы используйте `capitulation-reversal-v1`:
1h падение на `4%`/`6%` при объёме `2x`/`3x` от предыдущей 20-часовой средней
должно получить позднее зелёное восстановление выше close события в течение
12 часов. Стопом служит минимум события, выходы -- 1R, 2R или 48 часов.
Файлы с пропущенным, невалидным или полностью нулевым объёмом исключаются из
этого исследования; одиночные валидные минуты с нулевым объёмом сохраняются.

```bash
SUITE=capitulation-reversal-v1 SKIP_FETCH=1 ./scripts/run_research_v2.sh
```

Для проверки participation-confirmed breakout используйте
`volume-breakout-v1`: 1h close пересекает максимум предыдущих `20` или `40`
часов, а объём этой же завершённой свечи должен быть не ниже `1.5x` или `2.0x`
предыдущей 20-часовой средней. Предыдущий close не должен уже находиться выше
уровня. Стоп фиксирован на 1.5 ATR(14), выходы -- 1R, 3R или 7 суток. Для
этого suite файлы с отсутствующим, невалидным или полностью нулевым объёмом
исключаются; отдельные валидные нулевые минуты допускаются.

```bash
SUITE=volume-breakout-v1 SKIP_FETCH=1 ./scripts/run_research_v2.sh
```
