...
`development` по умолчанию ограничен `GOMEMLIMIT=512MiB GOGC=20`, чтобы
длинный прогон не был остановлен VPS по памяти. При достаточном объёме RAM
значения можно явно переопределить через одноимённые переменные окружения.
