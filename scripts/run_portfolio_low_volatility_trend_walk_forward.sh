#!/usr/bin/env bash
# Evaluate the two frozen low-volatility trend candidates on five pre-holdout
# windows. Inputs are read-only and the locked holdout remains unavailable.
...
RUN_ROOT="${RUN_ROOT:-$DATA_DIR/portfolio-runs/low-volatility-trend-walk-forward-$REVISION}"
...
if [[ -n "$TRACKED_STATUS" || -n "$UNTRACKED_SOURCE" ]]; then
  echo "ERROR: low-volatility trend workflow requires a clean Git worktree." >&2
  exit 1
fi
if [[ ! -x "$BIN" ]]; then
...
for top_k in "${CANDIDATES[@]}"; do
  candidate_dir="low-vol-trend-top${top_k}"
  for window in "${WINDOWS[@]}"; do
    ...
    run_logged "$BIN" research-validate portfolio-prepare \
      --strategy low-volatility-trend-v1 \
...
jq -s '[.[] | {
  ...
  decision
}]' "$RUN_ROOT"/low-vol-trend-*/*/report.json >"$RUN_ROOT/.summary.json.tmp"
mv "$RUN_ROOT/.summary.json.tmp" "$RUN_ROOT/summary.json"

echo
echo "Low-volatility trend walk-forward complete: $RUN_ROOT"
