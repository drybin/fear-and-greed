#!/usr/bin/env bash
# Evaluate the fixed BTC regime equal-weight top-50 baseline on five
# pre-holdout windows. It reads existing candles and never opens holdout.
...
RUN_ROOT="${RUN_ROOT:-$DATA_DIR/portfolio-runs/btc-regime-equal-weight-walk-forward-$REVISION}"
...
if [[ -n "$TRACKED_STATUS" || -n "$UNTRACKED_SOURCE" ]]; then
  echo "ERROR: BTC regime equal-weight workflow requires a clean Git worktree." >&2
  exit 1
fi
if [[ ! -x "$BIN" ]]; then
...
for window in "${WINDOWS[@]}"; do
  ...
  run_logged "$BIN" research-validate portfolio-prepare \
    --strategy btc-regime-equal-weight-v1 \
...
jq -s '[.[] | {
  ...
  decision
}]' "$RUN_ROOT"/*/report.json >"$RUN_ROOT/.summary.json.tmp"
mv "$RUN_ROOT/.summary.json.tmp" "$RUN_ROOT/summary.json"

echo
echo "BTC regime equal-weight walk-forward complete: $RUN_ROOT"
