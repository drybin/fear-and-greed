#!/usr/bin/env bash
# Evaluate the four frozen slow cross-sectional momentum candidates on five
# disjoint pre-holdout windows. No input data is downloaded or rewritten.
...
RUN_ROOT="${RUN_ROOT:-$DATA_DIR/portfolio-runs/slow-momentum-walk-forward-$REVISION}"
...
if [[ -n "$TRACKED_STATUS" || -n "$UNTRACKED_SOURCE" ]]; then
  echo "ERROR: slow momentum workflow requires a clean Git worktree." >&2
  exit 1
fi
if [[ ! -x "$BIN" ]]; then
...
for candidate in "${CANDIDATES[@]}"; do
  ...
  candidate_dir="momentum-${lookback}d-top${top_k}"
  for window in "${WINDOWS[@]}"; do
    ...
    run_logged "$BIN" research-validate portfolio-prepare \
      --strategy slow-cross-sectional-momentum-v1 \
...
jq -s '[.[] | {
  ...
  decision
}]' "$RUN_ROOT"/momentum-*/*/report.json >"$RUN_ROOT/.summary.json.tmp"
mv "$RUN_ROOT/.summary.json.tmp" "$RUN_ROOT/summary.json"

echo
echo "Slow momentum walk-forward complete: $RUN_ROOT"
