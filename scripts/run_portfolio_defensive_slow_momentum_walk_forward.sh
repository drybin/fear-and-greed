#!/usr/bin/env bash
# Evaluate defensive slow momentum on five pre-holdout windows. The workflow
# only reads the frozen candle CSVs and never opens the locked holdout.
...
RUN_ROOT="${RUN_ROOT:-$DATA_DIR/portfolio-runs/defensive-slow-momentum-walk-forward-$REVISION}"
...
if [[ -n "$TRACKED_STATUS" || -n "$UNTRACKED_SOURCE" ]]; then
  echo "ERROR: defensive slow momentum workflow requires a clean Git worktree." >&2
  exit 1
fi
if [[ ! -x "$BIN" ]]; then
...
CANDIDATES=("60:5" "60:10" "90:5" "90:10")
WINDOWS=(
...
for candidate in "${CANDIDATES[@]}"; do
  IFS=: read -r lookback top_k <<<"$candidate"
  candidate_dir="defensive-momentum-${lookback}d-top${top_k}"
  for window in "${WINDOWS[@]}"; do
    ...
    run_logged "$BIN" research-validate portfolio-prepare \
      --strategy defensive-slow-momentum-v1 \
...
jq -s '[.[] | {
  ...
  decision
}]' "$RUN_ROOT"/defensive-momentum-*/*/report.json >"$RUN_ROOT/.summary.json.tmp"
mv "$RUN_ROOT/.summary.json.tmp" "$RUN_ROOT/summary.json"

echo
echo "Defensive slow momentum walk-forward complete: $RUN_ROOT"
